...
SUPPORTED_MODELS = {
    "llama-70b": "huggingface_cache/quantized_llama_3.1_70B",  # Updated to your local path
...
class JudgeEvaluator:
    ...
    def evaluate_results(self, results_file: str, output_dir: str):
        ...
        # Load and extract results from new JSON structure
        with open(results_file, 'r') as f:
            file_data = json.load(f)
        
        # Extract the results list from the new structure
        results_list = file_data.get('results', [])
        if not isinstance(results_list, list):
            results_list = [results_list]
        
        # Store metadata for later
        input_metadata = file_data.get('metadata', {})
            ...
            for i in tqdm(range(0, len(results_list), batch_size)):
                ...
                for r in batch:
                    try:
                        prompts.append(self.create_evaluation_prompt(
                            ...
                            r.get('llm_rag_answer', '')
                        ))
                        # Store original texts for metrics
                        original_texts.append((r.get('original_answer', ''), r.get('llm_rag_answer', '')))
                    except Exception as e:
                        logger.error(f"Error creating prompt for qid {r.get('qid', 'unknown')}: {str(e)}")
                        prompts.append("")
                ...
                for result, response, (ref_text, gen_text) in zip(batch, responses, original_texts):
                    try:
                        ...
                        # Create evaluation entry with additional metadata
                        eval_entry = {
                            ...
                            'source_model_info': result.get('model_info', {}),  # Add source model info
                            'document_info': {  # Add document info
                                'document_id': result.get('document_id'),
                                'document_source': result.get('document_source'),
                                'document_focus': result.get('document_focus')
                            }
                        }
                        ...
                        logger.error(f"Error processing evaluation for qid {result.get('qid', 'unknown')}: {str(e)}")
                        continue
        ...
        # Save combined results with input metadata
        ...
        with open(base_output_dir / combined_filename, 'w') as f:
            json.dump({
                'metadata': {
                    ...
                    'input_metadata': input_metadata  # Include original input metadata
                },
...
def main():
    ...
    parser.add_argument("models", nargs="+", choices=["llama-70b", "llama-8b", "mixtral"], help="List of models to evaluate.")
